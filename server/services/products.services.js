...
module.exports = class ProductService {
	 ...
	 * @param {Response} res
	 */
	getAllProductsService = async (req, res) => {
		try {
	 ...
	 * @param {Response} res
	 */
	postProductService = async (req, res) => {
		let error_status = 500;
		try {
			if (Object.keys(req.body).length > 0) {
				const newProduct = await productHandler.save(req.body);
	 ...
	 * @param {Function} next
	 */
	productNotFoundService = async (req, res, next) => {
		try {
			let product = await productHandler.getById(parseInt(req.params.id));
			product
				? next()
	 ...
	 */
	initializeProductsService = async (PORT) => {
		let dummyData = [
			{
				id: 1,
